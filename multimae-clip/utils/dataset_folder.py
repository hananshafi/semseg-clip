...
from .nyu_class_mapping import get_class_names
...
class MultiTaskDatasetFolder(VisionDataset):
    ...
    def __init__(
        ...
        print("******************", os.path.join(self.root, f'{prefixes[self.tasks[0]]}{self.tasks[0]}'))
        samples = {
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if self.target_transform is not None:
            target = self.target_transform(target)
        #if 'semseg' in sample_dict.keys():
         #  class_names = get_class_names(sample_dict['semseg'])
         #  sample_dict['text_labels'] = torch.tensor(class_names)
        sample_dict['caption']=self.captions[index]
...
IMG_EXTENSIONS = ('.jpg', '.jpeg', '.png', '.ppm', '.bmp', '.pgm', '.tif', '.tiff', '.webp', '.jpx', '.PNG')
...
class MultiTaskImageFolder(MultiTaskDatasetFolder):
    ...
    def __init__(
        ...
        super(MultiTaskImageFolder, self).__init__(root, tasks, loader, IMG_EXTENSIONS if is_valid_file is None else None,
                                          ...
                                          is_valid_file=is_valid_file,
                                          prefixes=prefixes,
                                          max_images=max_images, caption_path=caption_path)
        self.imgs = self.samples
