...
class ConvNeXtAdapter(nn.Module):
    ...
    def init(self, dim_tokens_enc: int = 768):
        ...
        :param dim_tokens_enc: Dimension of tokens coming from encoder
        """
        self.in_channels = 768  #*2   #dim_tokens_enc * len(self.main_tasks)     #77*4 
    ...
    def forward(self, encoder_tokens: torch.Tensor, input_info: Dict,text_features):
        H, W = input_info['image_size']
        N_H, N_W = H // self.patch_size, W // self.patch_size
        ...
        x = self.blocks(x)
        b,c,h,w = x.shape
        x  = x.permute(0,2,3,1).reshape(-1, 384) @ (text_features.T)
        x =  x.reshape(b,40,h,w)
...
class DPTOutputAdapter(nn.Module):
    ...
    def init(self, dim_tokens_enc: int = 768):
        ...
        :param dim_tokens_enc: Dimension of tokens coming from encoder
        """
        self.dim_tokens_enc = 40 #* 2# dim_tokens_enc * len(self.main_tasks)
