...
class MultiMAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.avgpool = nn.AdaptiveAvgPool1d(384)
        ...
        self.clip_text_model = CustomCLIP(classnames=labelset, clip_model=self.clip_model)
        # for name, param in self.clip_text_model.named_parameters():
        #     if "prompt_learner" not in name:
        #         param.requires_grad_(False)
        # self.clip_text_model.to(device)
        #labelset = ade_20k_labels
        self.text_test = clip.tokenize(labelset)

        # self.clip_model , _ = clip.load("ViT-B/16", device=device)
        # self.clip_model.to(torch.float32)
        # self.clip_prompt_learner = CustomTextCLIP(self.clip_model)
...
class MultiViT(MultiMAE):
    ...
    def forward(self, x: Union[Dict[str, torch.Tensor], torch.Tensor], task_dict,is_train=True, return_all_layers=False, **kwargs):
        """
        Forward pass through input adapters, transformer encoder and output adapters.
        ...
        """
        # self.clip_text_model.to(x['rgb'].device)
        self.clip_model.to(x['rgb'].device)
        ...
        # with torch.no_grad():
        #     text_features = self.clip_model.encode_text(text.to(x['rgb'].device), is_train=True)
        #     text_test_features = self.clip_model.encode_text(self.text_test.to(x['rgb'].device), is_train=False)

        # clip_features  = self.clip_model.encode_image(x['rgb'])
        # # print("## clip_features dtype ## ", clip_features.dtype)
        # #encoder_tokens = torch.cat([encoder_tokens, clip_features], dim=2)
        # encoder_tokens = encoder_tokens + clip_features
        # # print("## encoder_tokens dtype ## ", encoder_tokens.dtype)
        #     #text_features = clip_model.encode_text(self.text.to(x['rgb'].device))
        # #text_features = text_features.squeeze(0)

        text_features = self.clip_text_model.forward()
        ...
        # encoder_tokens = encoder_tokens.reshape(-1, 768)
        ...
        # encoder_tokens = logits_scale * encoder_tokens@text_features.t()
        
        # encoder_tokens = encoder_tokens.reshape(b,n,40)
        # encoder_tokens = torch.cat([encoder_tokens_1, encoder_tokens_2], dim=2)
        
        # encoder_tokens = encoder_tokens.reshape(b,n,150)
        text_features = self.avgpool(text_features)
        b, n , c = encoder_tokens.shape
        # if is_train:
        #b, n , c = encoder_tokens.shape
        ...
        # ## text_features = text_features.permute(1,0).to(encoder_tokens.dtype)
        # logits_scale = self.logit_scale.exp()
        # encoder_tokens = logits_scale * encoder_tokens
        # encoder_tokens = logits_scale * encoder_tokens@(text_features.T)
        ...
        # else:
        #     #print("#### shape #### ", encoder_tokens.shape, text_test_features.shape,text_features.shape)
        #     encoder_tokens = self.logit_scale.exp() * (encoder_tokens.reshape(-1,512))@(text_test_features.T)
        #     encoder_tokens = self.avgpool(encoder_tokens.reshape(b,n,40))
        ...
        preds = {
            domain: self.output_adapters[domain](
                encoder_tokens=encoder_tokens,
                input_info=input_info,text_features=text_features
            )
            for domain in self.output_adapters
        }
        return preds
