...
class MultiViT(MultiMAE):
    ...
    def forward(self, x: Union[Dict[str, torch.Tensor], torch.Tensor], task_dict, return_all_layers=False, **kwargs):
        ...
        text = clip.tokenize(task_dict['caption'])
        ...
        #encoder_tokens = (encoder_tokens@text_features)     #self.logit_scale * #.to(torch.float32))                   #encoder_tokens +  clip_features           #torch.einsum('b n j , b n j -> b n j',encoder_tokens , clip_features)
    
        #encoder_tokens = self.cross_att(encoder_tokens,text_features.to(encoder_tokens.dtype))

        encoder_tokens = encoder_tokens[-4:]
        # encoder_tokens = [self.avgpool(en_t) for en_t in encoder_tokens]
        text_features = text_features[-4:]

        text_features = [t_feat.unsqueeze(0) for t_feat in text_features]
        encoder_tokens = [self.avgpool(en_tokens) for en_tokens in encoder_tokens]
        #text_features = text_features.permute(0,2,1).to(encoder_tokens.dtype)
