...
def get_args():
    ...
    parser.add_argument('--train_caption_path', default="/l/users/hanan.ghani/CVPR_2022/datasets/nyu_train_captions.txt", type=str,
                        help='captions for train')
    parser.add_argument('--val_caption_path', default="/l/users/hanan.ghani/CVPR_2022/datasets/nyu_val_captions.txt", type=str,
                        help='captions for val')
...
def main(args):
    ...
    dataset_train = build_semseg_dataset(args, data_path=args.data_path, caption_path=args.train_caption_path,transform=train_transform)
    dataset_val = build_semseg_dataset(args, data_path=args.eval_data_path, caption_path=args.val_caption_path,transform=val_transform, max_images=args.max_val_images)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, data_loader: Iterable,
    ...
    for step, (x, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        x_ = {key: value for key, value in x.items() if key!='caption'}
        tasks_dict = {
            task: tensor.to(device, non_blocking=True)
            for task, tensor in x_.items()
        }
        ...
        tasks_dict['caption'] = x['caption']
        # Forward + backward   
        with torch.cuda.amp.autocast(enabled=False):     #fp16
            preds = model(input_dict,  tasks_dict,is_train=True, return_all_layers=return_all_layers)
            seg_pred, seg_gt = preds['semseg'], tasks_dict['semseg']
            loss = criterion(seg_pred, seg_gt) #+ 0.2*(1 - loss_dot)
...
def evaluate(model, criterion, data_loader, device, epoch, in_domains, num_classes, dataset_name,
    ...
    for (x, _) in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast(enabled=fp16):
            preds = model(input_dict, tasks_dict,is_train=False, return_all_layers=return_all_layers)
            seg_pred, seg_gt = preds['semseg'], tasks_dict['semseg']
